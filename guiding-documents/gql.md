...
impl<'g> QueryPlanner<'g> {
            ...
            for element in &pattern.elements {
                if let PatternElement::Node(node) = element {
                    ...
                    for (key, value) in &node.properties {
                        if self.graph.has_index(&node.labels, key) {
                            return Ok(StartPoint::IndexLookup {
    ...
    fn build_traversal(
        ...
        query: &Query,
        start: StartPoint,
    ) -> Result<Box<dyn Traversal + 'g>, CompileError> {
        let g = self.graph.traversal();
        
        // Start traversal based on start point
        let mut traversal: Box<dyn Traversal> = match start {
            StartPoint::IndexLookup { labels, property, value, variable } => {
                let t = g.v()
                    .has_label_any(&labels)
                    .has_value(&property, value.into());
                Box::new(t)
            }
            StartPoint::LabelScan { labels, variable } => {
                Box::new(g.v().has_label_any(&labels))
            }
            StartPoint::FullScan => {
                Box::new(g.v())
            }
        ...
        for pattern in &query.match_clause.patterns {
            traversal = self.compile_pattern(traversal, pattern, &mut bindings)?;
        }
    ...
    fn compile_pattern(
        &self,
        mut traversal: Box<dyn Traversal>,
        pattern: &Pattern,
        bindings: &mut HashMap<String, BindingType>,
    ) -> Result<Box<dyn Traversal>, CompileError> {
        ...
        while let Some(element) = elements.next() {
            match element {
                PatternElement::Node(node) => {
                    // Apply label filter
                    if !node.labels.is_empty() {
                        traversal = Box::new(traversal.has_label_any(&node.labels));
                    }
                    
                    // Apply property filters
                    for (key, value) in &node.properties {
                        traversal = Box::new(traversal.has_value(key, value.clone()));
                    }
                    
                    // Bind variable
                    if let Some(var) = &node.variable {
                        traversal = Box::new(traversal.as_(var));
                        bindings.insert(var.clone(), BindingType::Vertex);
                    }
                }
                
                PatternElement::Edge(edge) => {
                    // Get next node
                    let next_node = match elements.next() {
                        Some(PatternElement::Node(n)) => n,
                        _ => return Err(CompileError::InvalidPattern),
                    };
                    
                    // Compile edge traversal
                    traversal = self.compile_edge(traversal, edge, next_node, bindings)?;
                }
            }
        }
    ...
    fn compile_edge(
        ...
        if let Some(quantifier) = &edge.quantifier {
            return self.compile_variable_path(traversal, edge, next_node, quantifier, bindings);
        }
        
        // Single hop
        let traversal: Box<dyn Traversal> = match edge.direction {
            EdgeDirection::Outgoing => {
                if labels.is_empty() {
                    Box::new(traversal.out())
                } else {
                    Box::new(traversal.out_labels(&labels))
                }
            }
            EdgeDirection::Incoming => {
                if labels.is_empty() {
                    Box::new(traversal.in_())
                } else {
                    Box::new(traversal.in_labels(&labels))
                }
            }
            EdgeDirection::Both => {
                if labels.is_empty() {
                    Box::new(traversal.both())
                } else {
                    Box::new(traversal.both_labels(&labels))
                }
            }
        };
        ...
        if let Some(var) = &edge.variable {
            bindings.insert(var.clone(), BindingType::Edge);
        }
        
        // Apply next node filters
        let mut traversal = traversal;
        
        if !next_node.labels.is_empty() {
            traversal = Box::new(traversal.has_label_any(&next_node.labels));
        }
        
        for (key, value) in &next_node.properties {
            traversal = Box::new(traversal.has_value(key, value.clone()));
        }
    ...
    fn compile_variable_path(
        ...
        // Build the repeated step
        let step: Box<dyn Fn(Box<dyn Traversal>) -> Box<dyn Traversal>> = match edge.direction {
            EdgeDirection::Outgoing => {
                if labels.is_empty() {
                    Box::new(|t| Box::new(t.out()))
                } else {
                    let labels = labels.clone();
                    Box::new(move |t| Box::new(t.out_labels(&labels)))
                }
            }
            EdgeDirection::Incoming => {
                if labels.is_empty() {
                    Box::new(|t| Box::new(t.in_()))
                } else {
                    let labels = labels.clone();
                    Box::new(move |t| Box::new(t.in_labels(&labels)))
                }
            }
            EdgeDirection::Both => {
                if labels.is_empty() {
                    Box::new(|t| Box::new(t.both()))
                } else {
                    let labels = labels.clone();
                    Box::new(move |t| Box::new(t.both_labels(&labels)))
                }
            }
        };
        
        // Apply repeat with bounds
        let min = quantifier.min.unwrap_or(1) as usize;
        let max = quantifier.max;
        
        let mut traversal = Box::new(traversal.repeat(step));
        
        if let Some(max) = max {
            traversal = Box::new(traversal.times(max as usize));
        }
        
        if min > 0 {
            traversal = Box::new(traversal.emit_after(min));
        } else {
            traversal = Box::new(traversal.emit());
        }
        
        // Apply end node filters
        if !next_node.labels.is_empty() {
            traversal = Box::new(traversal.has_label_any(&next_node.labels));
        }
    ...
    fn compile_expression(
        ...
        match expr {
            ...
            Expression::Variable(name) => {
                let name = name.clone();
                Ok(Box::new(move |ctx| ctx.get_binding(&name).unwrap().into()))
            }
            ...
            Expression::Aggregate { func, distinct, expr } => {
                // Aggregates are handled specially during execution
...
fn apply_binary_op(op: BinaryOperator, left: Value, right: Value) -> Value {
    match op {
        ...
        BinaryOperator::Lte => Value::Bool(left <= right),
        BinaryOperator::Gt => Value::Bool(left > right),
        BinaryOperator::Gte => Value::Bool(left >= right),
        BinaryOperator::And => {
...
| Pattern compiler | 3-4 days | Converting patterns to traversals |
| Expression compiler | 2-3 days | WHERE clause, projections |
| Aggregation support | 3-4 days | GROUP BY semantics |
| ORDER BY / LIMIT | 1-2 days | Post-processing |
| Testing | 3-4 days | Parser tests, execution tests |
| **Total** | **~4-5 weeks** | |

---

## 10. Limitations of Subset
...
| OPTIONAL MATCH | Not supported | Use separate queries |
