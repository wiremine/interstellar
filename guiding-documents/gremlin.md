# Interstellar: Gremlin Interface Implementation
...
The Gremlin interface provides external access to Interstellar's traversal engine through:
...
This enables interoperability with existing Gremlin clients (Python, JavaScript, Java, etc.) while leveraging Interstellar's high-performance traversal engine.
...
The bytecode interpreter converts Gremlin bytecode into Interstellar's internal traversal representation and executes it.
...
The optional Gremlin Server provides network access to Interstellar using the TinkerPop WebSocket protocol.
...
# Connect to Interstellar server
...
use interstellar::gremlin::{Bytecode, Instruction, Argument, GremlinInterpreter};
...
```rust
use interstellar::prelude::*;
use interstellar::gremlin::{parse_gremlin, GremlinInterpreter};
...
```rust
use interstellar::prelude::*;
use interstellar::gremlin::server::{GremlinServer, ServerConfig};
use std::sync::Arc;
...
| Behavior | TinkerPop | Interstellar |
|----------|-----------|-------------|
...
The Gremlin interface provides Interstellar with standard TinkerPop-compatible query capabilities:
...
The Gremlin interface, combined with the native Rust fluent API, provides users with flexible query options while maintaining the performance benefits of Interstellar's optimized traversal engine.
