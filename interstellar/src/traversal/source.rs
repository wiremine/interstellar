...
impl<'g, In> BoundTraversal<'g, In, Value> {
    ...
    /// Count all traversers (step version).
    ///
    /// This is a **barrier step** - it collects ALL input before producing
    /// a single `Value::Int` containing the count.
    ///
    /// This method returns a traversal (for chaining), unlike the terminal
    /// `count()` method which consumes the traversal and returns `u64`.
    ///
    /// # Example
    ///
    /// ```ignore
    /// // Count as step (can be chained)
    /// let result = g.v().count_step().next();
    /// // Returns: Some(Value::Int(5))
    /// ```
    ...
    /// Sum all numeric values (step version).
    ///
    /// This is a **barrier step** - it collects ALL input before producing
    /// a single value containing the sum.
    ///
    /// This method returns a traversal (for chaining), unlike the terminal
    /// `sum()` method which consumes the traversal and returns `Value`.
    ///
    /// # Example
    ///
    /// ```ignore
    /// // Sum as step (can be chained)
    /// let result = g.v().values("age").sum_step().next();
    /// ```
    ...
    /// Find the minimum value (step version).
    ///
    /// This is a **barrier step** - it collects ALL input before producing
    /// the minimum value.
    ///
    /// This method returns a traversal (for chaining), unlike the terminal
    /// `min()` method which consumes the traversal and returns `Option<Value>`.
    ///
    /// # Example
    ///
    /// ```ignore
    /// // Min as step (can be chained)
    /// let result = g.v().values("age").min_step().next();
    /// ```
    ...
    /// Find the maximum value (step version).
    ///
    /// This is a **barrier step** - it collects ALL input before producing
    /// the maximum value.
    ///
    /// This method returns a traversal (for chaining), unlike the terminal
    /// `max()` method which consumes the traversal and returns `Option<Value>`.
    ///
    /// # Example
    ///
    /// ```ignore
    /// // Max as step (can be chained)
    /// let result = g.v().values("age").max_step().next();
    /// ```
    ...
    /// Collect all traversers into a single list (step version).
    ///
    /// This is a **barrier step** - it collects ALL input before producing
    /// a single `Value::List` containing all values.
    ///
    /// This method returns a traversal (for chaining), unlike the terminal
    /// `fold()` method which consumes the traversal with a custom accumulator.
    ///
    /// # Example
    ///
    /// ```ignore
    /// // Fold into a list (can be chained)
    /// let result = g.v().values("name").fold_step().next();
    /// // Returns: Some(Value::List(["Alice", "Bob", ...]))
    /// ```
