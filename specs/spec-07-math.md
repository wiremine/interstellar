...
This specification details the implementation of the `MathStep` transform step for the RustGremlin graph database. The `math()` step evaluates mathematical expressions on traverser values, supporting both the current value (`_`) and labeled values from the path.
