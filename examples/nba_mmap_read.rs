//! NBA Graph - Memory-Mapped Storage Read Example
//!
//! This example demonstrates querying a persistent graph database using
//! the memory-mapped (MmapGraph) storage backend. It opens the NBA database
//! created by `nba_mmap_write` and runs the same comprehensive queries
//! as the in-memory `nba.rs` example.
//!
//! This proves that:
//! - Data persists across process restarts
//! - The MmapGraph backend provides identical query capabilities to InMemoryGraph
//! - The same traversal API works with both storage backends
//!
...
fn get_property(storage: &Arc<MmapGraph>, value: &Value, prop: &str) -> Option<String> {
...
fn get_name(storage: &Arc<MmapGraph>, value: &Value) -> String {
    get_property(storage, value, "name").unwrap_or_else(|| format!("{:?}", value))
}

/// Display a list of vertex results as names
fn display_names(storage: &Arc<MmapGraph>, results: &[Value]) -> String {
    if results.is_empty() {
...
fn display_with_prop(storage: &Arc<MmapGraph>, results: &[Value], prop: &str) -> String {
    if results.is_empty() {
        return "(none)".to_string();
    }
    results
...
fn main() {
    println!("=== NBA Graph Database - MmapGraph Read Example ===\n");
    ...
    let all_players = g.v().has_label("player").to_list();
    println!(
        "Players ({}): {}",
        all_players.len(),
        display_names(&storage, &all_players)
    );
    ...
    println!("\n=== MmapGraph Read Example Complete ===");
    println!(
        "\nThis data was read from the persistent database at {}",
        DB_PATH
    );
    println!("The same queries work identically with both InMemoryGraph and MmapGraph!");
}
